...
<body class="bg-cream text-forest overflow-x-hidden">
  ...
  <nav class="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-[6%] h-[72px] bg-forest/95 backdrop-blur-md border-b border-leaf/10">
    ...
    <ul class="flex items-center gap-8 list-none">
      ...
      <li><a href="products.php" class="text-cream/75 hover:text-gold text-sm font-medium tracking-widest uppercase">Produk</a></li>
      <li class="relative">
        <a href="cart.php" class="text-cream text-xl hover:text-gold transition">🛒
          <?php if(isset($_SESSION['cart']) && count($_SESSION['cart']) > 0): ?>
            <span class="absolute -top-2 -right-3 bg-gold text-forest text-xs font-bold px-2 rounded-full"><?= count($_SESSION['cart']) ?></span>
          <?php endif; ?>
        </a>
      </li>
      <li><a href="logout.php" class="bg-red-500 text-white text-[10px] px-3 py-1.5 rounded-full hover:bg-red-600">Logout</a></li>
    </ul>
  </nav>

    </ul>

    <div id="mobile-menu" class="hidden absolute top-[72px] left-0 w-full bg-forest flex-col p-6 gap-4 lg:hidden border-b border-leaf/20 shadow-xl">
      <a href="index.php" class="text-gold font-bold uppercase tracking-widest">Home</a>
      <a href="profil.html" class="text-cream font-bold uppercase tracking-widest">Tentang</a>
      <a href="contact.html" class="text-cream font-bold uppercase tracking-widest">Kontak</a>
      <a href="cart.php" class="text-cream font-bold uppercase tracking-widest flex items-center gap-2">🛒 Keranjang (<?= isset($_SESSION['cart']) ? count($_SESSION['cart']) : 0 ?>)</a>
      <a href="logout.php" class="text-red-400 font-bold uppercase tracking-widest">Logout</a>
    </div>
  </nav>
